...
App::App(){
    if(RuntimeConfig::instance()->has("NOON_SERVER_PORT")){
        _port = parse_int(RuntimeConfig::instance()->get("NOON_SERVER_PORT"));
    }
    instance = this;
...
bool App::loadConfig() {
    ...
    std::cout << "try setting the --appDir parameter to your app directory (the one with the `noon.config.lua` file)" << std::endl;
    printHelp();
    return false;
}
